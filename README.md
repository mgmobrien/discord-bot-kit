...
Python standard library only — nothing to install, no virtualenv, no background process. One file, nine commands.
...
It requests the permissions its commands use — view, read history, send, send-in-threads, add reactions, embed links, attach files, external emoji. **Requesting is not receiving:** the server's admin sees that request, can trim it on approval, and separately controls what the bot can do per channel. Discord enforces the intersection. [SETUP.md](SETUP.md) lists every permission with the command that needs it.

- It reaches **only** servers that have explicitly invited it. There is no crawling and no public read.
- It cannot delete messages, remove other people's reactions, create or manage threads, kick anyone, manage roles, or change the server.
